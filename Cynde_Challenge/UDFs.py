...
def generate_companies_embeddings(df: pd.DataFrame, embeddings_model) -> [pd.DataFrame, np.ndarray]:
    """
    ...
    df["concatenated_name_description"] = df["Name"].fillna(" ").astype(str) + " "+ df["Description"].fillna(" ").astype(str)
    
    # Clean the contancated name description
    df["concatenated_cleaned_text"] = df["concatenated_name_description"].apply(clean_text)

    # Generate embeddings
    embeddings = embeddings_model.encode(df["concatenated_cleaned_text"], show_progress_bar = False, 
                                device = "cude" if embeddings_model.device.type == "cuda" else "cpu")
    df["embeddings"] = embeddings.tolist()  # Convert to list for DataFrame compatibility
